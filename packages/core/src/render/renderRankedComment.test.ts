...
import { MAX_LISTED, renderRankedComment } from "./renderRankedComment.js";
...
describe("renderComment (R3, R4)", () => {
  ...
  it("handles a tier mix with no High without throwing", () => {
    const out = renderRankedComment([chunk("a.ts", "Medium"), chunk("b.ts", "Low")]);
    expect(out).toContain("**[Medium]**");
    expect(out).toContain("Plus 1 lower-risk hunk not listed.");
  });
  ...
  it("orders High before Medium", () => {
    const out = renderRankedComment([chunk("med.ts", "Medium"), chunk("high.ts", "High")]);
  ...
  it(`caps the listed flagged chunks at ${MAX_LISTED} on a large PR`, () => {
    ...
    const out = renderRankedComment([...many, chunk("src/lib/low.ts", "Low")]);
  ...
  it("does not cap when flagged chunks fit within the budget", () => {
    const out = renderRankedComment([chunk("a.ts", "High"), chunk("b.ts", "Medium")]);
  ...
  it("never uses merge-gating language (advisory only)", () => {
    ...
    expect(renderRankedComment([])).not.toMatch(NO_MERGE_WORDS);
  });
});
...
describe("renderComment — reaction affordance (R3)", () => {
  ...
  it("encodes the chunk fingerprint and tier in the reaction links", () => {
    const out = renderRankedComment([chunk("src/auth/login.ts", "High")], REACTIONS);
  ...
  it("does not add an affordance to the Low remainder line", () => {
    const out = renderRankedComment([chunk("a.ts", "High"), chunk("b.ts", "Low")], REACTIONS);
  ...
  it("renders no affordance when reactions are not configured", () => {
    const withReactions = renderRankedComment([chunk("a.ts", "High")], REACTIONS);
