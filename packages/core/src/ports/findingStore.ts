 ...
 *
 * `replaceForPr` is the only writer: each review run replaces the PR's findings
 * with its own set. A re-pushed PR fires a fresh run, so this keeps exactly one
 * run's findings live — no stale rows from earlier runs, and no duplicate
 * fingerprints colliding in the card view's keys.
 */
export interface FindingStore {
  replaceForPr(ref: FindingPrRef, findings: ReviewFinding[]): Promise<void>;
