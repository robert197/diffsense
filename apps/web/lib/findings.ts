...
/**
 * Findings for one PR, ordered by risk (rank ascending, id ascending to break
 * ties). The writer (`FindingStore.replaceForPr`) keeps only the newest run's
 * findings per PR, so every fingerprint appears once and the rows here are a
 * single run. The card view renders them in this order — it never re-sorts.
 */
